...
namespace whfc_rb {
    ...
    template<>
    class FlowHypergraphBuilderExtractor<whfc_pr::LawlerFlowHypergraphParallel> {
    public:
        template<typename T>
        using vec = std::vector<T, tbb::scalable_allocator<T>>;

        using Hypergraph = whfc_pr::LawlerFlowHypergraphParallel;
        static constexpr NodeID invalid_node = std::numeric_limits<NodeID>::max();
        Hypergraph fhgb;
    ...
    template<>
    class FlowHypergraphBuilderExtractor<whfc_pr::LawlerFlowHypergraph> {
    public:
        template<typename T>
        using vec = std::vector<T, tbb::scalable_allocator<T>>;

        using Hypergraph = whfc_pr::LawlerFlowHypergraph;
        static constexpr NodeID invalid_node = std::numeric_limits<NodeID>::max();
        Hypergraph fhgb;
