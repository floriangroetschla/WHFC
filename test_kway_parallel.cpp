...
int main(int argc, const char *argv[]) {
    ...
    if (argc != 11) {
        throw std::runtime_error("Usage ./KWayRefinementParallel HypergraphFile epsilon k seed preset numThreads useThreadPinning(0 or 1) distancePiercing(0 or 1) maxNumIterations mode");
    }
    ...
    if (!mode.compare("seqExtraction_seqDinic")) {
        ...
        whfc_rb::KWayRefinerParallel<whfc_rb::PartitionThreadsafe, whfc::FlowHypergraphBuilder, whfc::Dinic, whfc_rb::HypergraphBuilderExtractor<whfc::FlowHypergraphBuilder, whfc_rb::PartitionThreadsafe>> refiner(partition, timer, mt, config);
        ...
        whfc_rb::KWayRefinerParallel<whfc_rb::PartitionThreadsafe, whfc::FlowHypergraphBuilder, whfc::DinicThreadLocalVectors, whfc_rb::HypergraphBuilderExtractor<whfc::FlowHypergraphBuilder, whfc_rb::PartitionThreadsafe>> refiner(partition, timer, mt, config);
        numIterations = refiner.refine(epsilon, maxNumIterations);
    } else if (!mode.compare("parExtraction_parDinic")) {
        std::cout << "mode parExtraction_parDinic" << std::endl;
        whfc_rb::KWayRefinerParallel<whfc_rb::PartitionThreadsafe, whfc::FlowHypergraphBuilder, whfc::DinicThreadLocalVectors, whfc_rb::FlowHypergraphBuilderExtractorParallel<whfc::FlowHypergraphBuilder, whfc_rb::PartitionThreadsafe>> refiner(partition, timer, mt, config);
        numIterations = refiner.refine(epsilon, maxNumIterations);
    } else if (!mode.compare("seqExtraction_seqPR")) {
        std::cout << "mode seqExtraction_seqPR" << std::endl;
        whfc_rb::KWayRefinerParallel<whfc_rb::PartitionThreadsafe, whfc_pr::LawlerFlowHypergraph, whfc_pr::PushRelabel, whfc_rb::HypergraphBuilderExtractor<whfc_pr::LawlerFlowHypergraph, whfc_rb::PartitionThreadsafe>> refiner(partition, timer, mt, config);
        numIterations = refiner.refine(epsilon, maxNumIterations);
    } else if (!mode.compare("parExtraction_seqPR")) {
        std::cout << "mode parExtraction_seqPR" << std::endl;
        whfc_rb::KWayRefinerParallel<whfc_rb::PartitionThreadsafe, whfc_pr::LawlerFlowHypergraph, whfc_pr::PushRelabel, whfc_pr::LawlerFlowHypergraphBuilderExtractorParallel<whfc_pr::LawlerFlowHypergraph, whfc_rb::PartitionThreadsafe>> refiner(partition, timer, mt, config);
        numIterations = refiner.refine(epsilon, maxNumIterations);
    } else if (!mode.compare("seqExtraction_parPR")) {
        std::cout << "mode seqExtraction_parPR" << std::endl;
        whfc_rb::KWayRefinerParallel<whfc_rb::PartitionThreadsafe, whfc_pr::LawlerFlowHypergraphParallel, whfc_pr::PushRelabelParallel, whfc_rb::HypergraphBuilderExtractor<whfc_pr::LawlerFlowHypergraphParallel, whfc_rb::PartitionThreadsafe>> refiner(partition, timer, mt, config);
        numIterations = refiner.refine(epsilon, maxNumIterations);
    } else if (!mode.compare("parExtraction_parPR")) {
        std::cout << "mode parExtraction_parPR" << std::endl;
        whfc_rb::KWayRefinerParallel<whfc_rb::PartitionThreadsafe, whfc_pr::LawlerFlowHypergraphParallel, whfc_pr::PushRelabelParallel, whfc_pr::LawlerFlowHypergraphBuilderExtractorParallel<whfc_pr::LawlerFlowHypergraphParallel, whfc_rb::PartitionThreadsafe>> refiner(partition, timer, mt, config);
        numIterations = refiner.refine(epsilon, maxNumIterations);
    } else {
