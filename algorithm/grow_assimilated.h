...
template<>
class GrowAssimilated <whfc_pr::PushRelabelParallel> {
public:
    using Pin = whfc_pr::FlowHypergraph::Pin;

    using ScanList = typename whfc_pr::PushRelabelParallel::ScanList;
    using ReachableNodes = typename whfc_pr::PushRelabelParallel::ReachableNodes;
    using ReachableHyperedges = typename whfc_pr::PushRelabelParallel::ReachableHyperedges;


    static void grow(CutterState<whfc_pr::PushRelabelParallel>& cs, ScanList& nodes_to_scan, const bool reach_and_settle = false) {
        unused(reach_and_settle);

        ReachableNodes& n = cs.n;
        ReachableHyperedges& h = cs.h;
        nodes_to_scan.clear();
        whfc_pr::FlowHypergraph& hg = cs.hg;
            ...
            for (const auto& he_inc : hg.hyperedgesOf(u)) {
                ...
                if (!h.areAllPinsSources(e)) {
                    ...
                    if (scanAllPins) {
                        ...
                        if (whfc_pr::PushRelabelParallel::grow_reachable_marks_flow_sending_pins_when_marking_all_pins) {
                            assert(h.areFlowSendingPinsSourceReachable(e) || reach_and_settle);
                            if (!h.areFlowSendingPinsSources(e))
                                cs.settleFlowSendingPins(e);
                        }
                        ...
                        assert(h.areFlowSendingPinsSourceReachable(e) || reach_and_settle || (!whfc_pr::PushRelabelParallel::same_traversal_as_grow_assimilated && h.areAllPinsSourceReachable(e)));
#ifndef NDEBUG
                        if (!whfc_pr::PushRelabelParallel::same_traversal_as_grow_assimilated && h.areAllPinsSourceReachable(e) && !h.areFlowSendingPinsSourceReachable(e)) {
							h.reachFlowSendingPins(e);
						}
#endif
                        cs.settleFlowSendingPins(e);
    ...
    template<>
    class GrowAssimilated <whfc_pr::PushRelabel> {
    public:
        using Pin = whfc_pr::FlowHypergraph::Pin;

        using ScanList = typename whfc_pr::PushRelabel::ScanList;
        using ReachableNodes = typename whfc_pr::PushRelabel::ReachableNodes;
        using ReachableHyperedges = typename whfc_pr::PushRelabel::ReachableHyperedges;


        static void grow(CutterState<whfc_pr::PushRelabel>& cs, ScanList& nodes_to_scan, const bool reach_and_settle = false) {
            unused(reach_and_settle);
                ...
                for (const auto& he_inc : hg.hyperedgesOf(u)) {
                    ...
                    if (!h.areAllPinsSources(e)) {
                        ...
                        if (scanAllPins) {
                            ...
                            if (whfc_pr::PushRelabel::grow_reachable_marks_flow_sending_pins_when_marking_all_pins) {
                                assert(h.areFlowSendingPinsSourceReachable(e) || reach_and_settle);
                                if (!h.areFlowSendingPinsSources(e))
                                    cs.settleFlowSendingPins(e);
                            }
                            ...
                            assert(h.areFlowSendingPinsSourceReachable(e) || reach_and_settle || (!whfc_pr::PushRelabel::same_traversal_as_grow_assimilated && h.areAllPinsSourceReachable(e)));
#ifndef NDEBUG
                            if (!whfc_pr::PushRelabel::same_traversal_as_grow_assimilated && h.areAllPinsSourceReachable(e) && !h.areFlowSendingPinsSourceReachable(e)) {
                                h.reachFlowSendingPins(e);
                            }
#endif
                            cs.settleFlowSendingPins(e);
